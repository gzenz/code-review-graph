...
class _GraphFixture:
    ...
    def _add_tested_by(self, test_qn: str, target_qn: str, path: str) -> None:
        # Upstream stores TESTED_BY as source=production, target=test (#515),
        # so a production node finds its tests by source. The args stay
        # (test_qn, target_qn=production) for readability; the edge is built
        # in the corrected direction.
        edge = EdgeInfo(
            kind="TESTED_BY", source=target_qn, target=test_qn,
